...
public:
	...
	Data(std::vector<int> a) {
		x_coord = std::move(a);
	}
	Data(std::vector<int> a, std::vector<int> b) {
		x_coord = std::move(a);
		y_coord = std::move(b);
	}
	Data(std::vector<int> a, std::vector<int> b, std::vector<int> c) {
		x_coord = std::move(a);
		y_coord = std::move(b);
		z_coord = std::move(c);
	}
...
class Shape
{
	...
	void what_figure_set(Data data);
	int type;
public:
	...
	Figure* figure;
	Shape() = default;
	Shape(int type, Data data);

	double volume;
	double square;

	std::vector<int> getXco();
	std::vector<int> getYco();
	std::vector<int> getZco();
	std::vector<std::vector<int>> getFullCoord();
	void CallsetCoord(int os, int index, int setting);
